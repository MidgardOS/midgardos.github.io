...
# Expanded from the Fedora lesspipe.sh by Gary Greene <greeneg@yggdrasilsoft.com>
#
...
case ${type#"$TMPF_pre": } in
    *tar\ archive*)
        if [ -x "`/usr/bin/which tar 2>/dev/null`" ]; then
            ...
            echo "tar is not available for preprocessing" 1>&2
            rm -f "$TMPF"
            TMPF="$TMPF_pre"
        fi
        ;;
    *Microsoft\ Cabinet\ *\ data*)
        if [ -x "`/usr/bin/which cabextract 2>/dev/null`" ]; then
            ...
            echo "cabextract is not available for preprocessing" 1>&2
            rm -f "$TMPF"
            TMPF="$TMPF_pre"
        fi
        ;;
    *RPM*)
        if [ -x "`/usr/bin/which rpm 2>/dev/null`" ]; then
            ...
            echo "rpm is not available for preprocessing" 1>&2
            rm -f "$TMPF"
            TMPF="$TMPF_pre"
        fi
		;;
    *DVI*)
        if [ -x "`/usr/bin/which dvi2tty 2>/dev/null`" ]; then
            if [ "${TMPF_pre%.dvi}" != "$TMPF_pre" ] ; then
                ...
                echo "dvi2tty requires an input file name with the suffix .dvi" 1>&2
                rm -f "$TMPF"
                TMPF="$TMPF_pre"
            fi
        else
            echo "dvi2tty is not available for preprocessing" 1>&2
            rm -f "$TMPF"
            TMPF="$TMPF_pre"
        fi
        ;;
    *PDF*)
        if [ -x "`/usr/bin/which pdftotext 2>/dev/null`" ]; then
            ...
            echo "pdftotext is not available for preprocessing" 1>&2
            rm -f "$TMPF"
            TMPF="$TMPF_pre"
        fi
		;;
    *Debian\ binary\ package*)
        if [ -x "`/usr/bin/which dpkg-deb 2>/dev/null`" ]; then
            ...
            echo "dpkg-deb is not available for preprocessing" 1>&2
            rm -f "$TMPF"
            TMPF="$TMPF_pre"
        fi
        ;;
    *\ ar\ archive*)
        if [ -x "`/usr/bin/which nm 2>/dev/null`" ]; then
            ...
            echo "nm is not available for preprocessing" 1>&2
            rm -f "$TMPF"
            TMPF="$TMPF_pre"
        fi
        ;;
    ...
    *diff\ output*)
        ...
        if [ $R_NOT_SET = false ] && [ -x "`/usr/bin/which colordiff 2>/dev/null`" ]; then
            ...
            case ${type#"$TMPF_pre": } in
                *troff*)
                    if [ -x "`/usr/bin/which groff 2>/dev/null`" ]; then
                        ...
                        echo "groff is not available for preprocessing" 1>&2
                        rm -f "$TMPF"
                        TMPF="$TMPF_pre"
                    fi
                    ;;
                *PostScript*)
                    if [ -x "`/usr/bin/which ps2ascii 2>/dev/null`" ]; then
                        ...
                        echo "ps2ascii is not available for preprocessing" 1>&2
                        rm -f "$TMPF"
                        TMPF="$TMPF_pre"
                    fi
                    ;;
                *HTML*)
                    if [ -x "`/usr/bin/which w3m 2>/dev/null`" ]; then
                        ...
                        echo "lynx/w3m not available for preprocessing" 1>&2
                        rm -f "$TMPF"
                        TMPF="$TMPF_pre"
                    fi
                    ;;
